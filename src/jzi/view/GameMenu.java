...
public class GameMenu implements Menu {
    ...
    private JLabel die;
    /**
     * Label for the current object to place.
     */
    private JLabel currentToPlaceLabel;
    /**
     * Text label for the current object to place.
     */
    private JLabel currentToPlaceLabelText;
    /**
    ...
    /**
     * List of objects left to place.
     */
    private LinkedList<Image> currentToPlaceList = new LinkedList<Image>();
    /**
     * Image of an ammo.
     */
    ...
    /**
     * Iterator over a List of ammo,life,zombie images.
     */
    private Iterator<Image> currentToPlace;
    /**
     * Text for ammo use.
     */
    ...
    private void addToPanel() {
        ...
        tilePanel.add(currentToPlaceLabelText);
        tilePanel.add(currentToPlaceLabel, "wrap");
    }
    ...
    private void createUpdateMap() {
        ...
        updateMap.put(Update.PlaceObjects, new PlaceObjectUpdate());
        updateMap.put(Update.CurrentToPlaceUpdate, new CurrentToPlaceUpdate());
        updateMap.put(Update.CurrentToPlaceReset, new CurrentToPlaceReset());
    ...
    private void createLabelsandButtons(IGame game) {
        ...
        life = new JLabel("0");
        points = new JLabel("0");
        currentToPlaceLabel = new JLabel();
        currentToPlaceLabelText = new JLabel(words.next());
        revives = new JLabel();
        reviveValue = words.next();
        ammoUse = words.next();
        ammoUse2 = words.next();
        lifeUse = words.next();
        currentToPlaceLabelText.setVisible(false);
    ...
    /**
     * Update that gets called when an object needs to be placed.
     * 
     * @author Tobias Groth
     * 
     */
    private class CurrentToPlaceUpdate implements ViewUpdate {
        @Override
        public void execute(Observable o) {
            TileState tileState = (TileState) (((Game) o).getCurrentState());
            setCurrentToPlace(false);
            setInfoPane(tileState);
        }
    }

    /**
     * Update that gets called when the info pane needs to be reset.
     * 
     * @author Tobias Groth
     * 
     */
    private void setInfoPane(TileState tileState) {
        revives.setVisible(false);
        ammo.setText(new Integer(tileState.getAmmoCount()).toString());
        life.setText(new Integer(tileState.getLifeCount()).toString());
        points.setText(new Integer(tileState.getZombieCount()).toString());
        infoPane.setVisible(true);
    }

    /**
     * Update that gets called when no more objects are to be placed.
     * 
     * @author Tobias Groth
     * 
     */
    private class CurrentToPlaceReset implements ViewUpdate {
        @Override
        public void execute(Observable o) {
            setCurrentToPlace(true);
        }
    }
    ...
    /**
     * Update that gets called when an object is placed.
     * 
     * @author Tobias Groth
     * 
     */
    private class PlaceObjectUpdate implements ViewUpdate {
    ...
    /**
     * Sets the object type that needs to be placed.
     * 
     * @param reset
     *            whether or not to reset the view
     */
    private void setCurrentToPlace(boolean reset) {
        if (!reset) {
            currentToPlaceLabelText.setVisible(true);
            currentToPlaceLabel.setIcon(new ImageIcon(currentToPlace.next()));
        } else {
            currentToPlace = currentToPlaceList.iterator();
            currentToPlaceLabelText.setVisible(false);
            currentToPlaceLabel.setIcon(null);
        }
    }
    ...
    private void readImages() {
        ...
        currentToPlaceList.add(zombieImage.getScaledInstance(50, 50, 0));
        currentToPlaceList.add(ammoImage.getScaledInstance(50, 50, 0));
        currentToPlaceList.add(lifeImage.getScaledInstance(50, 50, 0));
        currentToPlace = currentToPlaceList.iterator();
    }
    ...
    private void setLanguage(Iterator<String> words, String language) {
        ...
        currentToPlaceLabelText.setText(words.next());
        reviveValue = words.next();
        ammoUse = words.next();
        ammoUse2 = words.next();
        lifeUse = words.next();
